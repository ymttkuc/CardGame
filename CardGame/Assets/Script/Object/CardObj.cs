...
public class CardObj : MonoBehaviour {
    ...
    public static Vector2 size = new Vector2(2.5f, 3.5f);
    ...
    private void Awake() {
        ...
        for (int i = 0; i < (int)Card.State.size; ++i) {
            Destroy(stateNum[i]);
            stateNum[i] = Instantiate(orig_num,
            ...
            stateNum[i].transform.localScale = new Vector3(numSize, numSize);
            stateNum[i].SetActive(true);
        }
    ...
    public void RefrectState() {
        ...
        for (int i = 0; i < (int)Card.State.size; ++i) {
            ...
            stateNum[i].GetComponent<Number>().num = now;
            if (now < orig) {
                stateNum[i].GetComponent<Number>().isBuff = true;
                stateNum[i].GetComponent<Number>().isDebuff = false;
            } else if (now > orig) {
                stateNum[i].GetComponent<Number>().isBuff = false;
                stateNum[i].GetComponent<Number>().isDebuff = true;
            } else {
                stateNum[i].GetComponent<Number>().isBuff = false;
                stateNum[i].GetComponent<Number>().isDebuff = false;
            }
        ...
        if (child_name.GetComponent<TextMesh>().text == "") {
            if (card.GetCardType() == Card.Type.character) {
                stateNum[(int)Card.State.cost].GetComponent<Number>().isVisible = false;
            } else {
                stateNum[(int)Card.State.cost].GetComponent<Number>().isVisible = true;
            }
    ...
    public void LayerSet(int _num, string _tag) {
        int num = _num * CardLayer.GetAllSize();
        for (int i = 0; i < (int)CardLayer.Obj.size + CardLayer.GetChromaSize(); ++i) {
            GameObject gameObject = null;
            if (i < (int)CardLayer.Obj.size) {
                switch ((CardLayer.Obj)i) {
                case CardLayer.Obj.picture: gameObject = child_picture; break;
                case CardLayer.Obj.chroma: gameObject = child_chroma; break;
                case CardLayer.Obj.cfr: break;
                case CardLayer.Obj.name: gameObject = child_namePane; break;
                case CardLayer.Obj.toughnessView: gameObject = child_stateView[(int)Card.State.toughness]; break;
                case CardLayer.Obj.powerView: gameObject = child_stateView[(int)Card.State.power]; break;
                case CardLayer.Obj.costView: gameObject = child_stateView[(int)Card.State.cost]; break;
                case CardLayer.Obj.toughenssNum: gameObject = stateNum[(int)Card.State.toughness]; break;
                case CardLayer.Obj.powerNum: gameObject = stateNum[(int)Card.State.power]; break;
                case CardLayer.Obj.costNum: gameObject = stateNum[(int)Card.State.cost]; break;
    ...
    public static Card.Color GetStartColor(int[] _color) {
        Card.Color re = Card.Color.size;

        for (int i = 0; i < (int)Card.Color.size; ++i) {
            if (_color[i] > 0) {
                if (_color[(i + (int)Card.Color.size - 1) % (int)Card.Color.size] == 0) {
                    ...
                    for (int j = 0; j < (int)Card.Color.size; ++j) {
                        ...
                        for (int k = 0; k < (int)Card.Color.size; ++k) {
                            if (_color[(i + j + k + (int)Card.Color.size)
    ...
    public static Card.Color GetStartColor(Card _card) {
        return GetStartColor(_card.GetColor());
    }
    ...
    public static void SetCardObjs(Card[] _cards, ref List<GameObject> _orig_cards) {
        ...
        for (int i = 0; i < _cards.Length; ++i) {
            if (_orig_cards.Count == i) { _orig_cards.Add(CardObj.CreatePrefab()); }
...
public class CardLayer {
    ...
    public enum ChromaObj {
        view,
        num,
        size
    }
    public static int GetChromaSize() { return (int)ChromaColor.size * (int)ChromaObj.size; }
    ...
    public static int GetAllSize() {
        return (int)Obj.size + (int)ChromaColor.size * (int)ChromaObj.size + (int)View.size;
    }
}
