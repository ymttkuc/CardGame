...
public class OpponentHand : MonoBehaviour {
 ...
 //   int pre_hand_num;   //さっきまでの手札枚数
    ...
    public void SetCardNum(int _num) {
        if (_num < 0) { _num = 0; }
        if (GameScript.HAND_MAX < _num) { _num = GameScript.HAND_MAX; }
        for (int i = hand.Count; _num < i; --i) {
            Destroy(hand[i]); hand.RemoveAt(i);
        }
        while (hand.Count < _num) {
            hand.Add(CardObj.CreatePrefab());
        }
    }
