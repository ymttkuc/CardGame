...
public static class Utility {
    ...
    //Vector3のcosを返す
    public static float GetVector3Cos(Vector3 _v1, Vector3 _v2) {
        return Vector3.Dot(_v1, _v2) / (Vector3.Magnitude(_v1) * Vector3.Magnitude(_v2));
    }

    //Vector3をVector3で複製する
    //どうやら
    //var hoge = new Vector3(3, 4, 5);
    //hogehoge = hoge;
    //でディープコピーになるらしい
    ...
    public static GameObject CreateEmpty(Vector3 _position, Quaternion _rotation) {
        GameObject obj = new GameObject();
        obj.transform.position = _position;
        obj.transform.rotation = _rotation;
        return obj;
    }
    public static GameObject CreateEmpty(Transform _parent) {
        return CreateEmpty(_parent.position, _parent.rotation, _parent);
    }
    public static GameObject CreateEmpty(Vector3 _position, Quaternion _rotation, Transform _parent) {
        GameObject obj = new GameObject();
        obj.transform.position = _position;
        obj.transform.rotation = _rotation;
