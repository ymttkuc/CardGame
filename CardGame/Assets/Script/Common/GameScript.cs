...
public class GameScript {
    ...
    public const int HAND_MAX = 15; //手札の上限枚数
    public const int WARNING_HAND_MAX = 13; //手札の上限警告
    ...
    int _FuncMove(int _num, Zone _fromZone, Zone _toZone, int _toNum) {
        ...
        if (moveTo == Zone.hand) {
            if (zone[cards[_num].GetControl()][(int)moveTo].Count == GameScript.HAND_MAX) {
    ...
    public int FuncCreate(int _player, Zone _zone, int _id, int _num) {
        ...
        //移動先に移動できるかどうかを確かめる
        //移動できない場合、代わりの移動先を用意する
        if (to == Zone.hand && zone[_player][(int)to].Count == GameScript.HAND_MAX) {
